BASE_URL="https://www.eater.com/archives/"
N_ARTICLE_LINK_PAGES = 15
OUTPUT_FILE = 'article-lists/eater-article-urls.json'
WORKER_THREADS = 5
        ...
        for i in self.chunk:
            try:
                ...
                for article in articles: 
                    link = article.find('a', {'class': 'c-entry-box--compact__image-wrapper'})
                    area = article.find('h2', {'class': "c-entry-box--compact__title"})
                    title = area.find('a')
                    if title is None or title.string is None:
                        continue
                    article_url = EaterArticleUrl(url=link['href'], title=str(title.string.strip()) or '')
