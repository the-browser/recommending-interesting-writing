BASE_URL="https://observer.com/national-politics/page/"
N_ARTICLE_LINK_PAGES = 30
OUTPUT_FILE = 'article-lists/observer-article-urls.json'
WORKER_THREADS = 16
        ...
        for i in self.chunk:
            try:
                ...
                for article in articles: 
                    link = article.find('a')
                    title = article.find('h2', {'class': 'entry-title'})
                    if title is None or title.string is None:
                        continue
                    article_url = ObserverArticleUrl(url=link['href'], title=str(title.string.strip()) or '')
