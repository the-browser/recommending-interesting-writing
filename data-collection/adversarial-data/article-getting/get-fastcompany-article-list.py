BASE_URL="https://www.fastcompany.com/category/creativity/"
N_ARTICLE_LINK_PAGES = 130
OUTPUT_FILE = '../article-lists/fastcompany-article-urls.json'
WORKER_THREADS = 16
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                article_list_page = get(f"{BASE_URL}")
                soup = BeautifulSoup(article_list_page.text, "html5lib")
                articles = soup.find_all('article', {'class': 'card'})
                for article in articles: 
                    link = article.find('a')
                    title = article.find('h2', {'class': 'card__title'})
                    print(title.string.strip())
                    if title is None or title.string is None:
                        continue
                    article_url = AeonArticleUrl(url="https://www.fastcompany.com" + link['href'], title=str(title.string.strip()) or '')
