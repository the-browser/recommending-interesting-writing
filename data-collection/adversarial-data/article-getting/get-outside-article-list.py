BASE_URL="https://www.outsideonline.com/archive/long-reads?page="
N_ARTICLE_LINK_PAGES = 56
OUTPUT_FILE = '../article-lists/outside-article-urls.json'
WORKER_THREADS = 16
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                articles = soup.find_all('article', {'class': 'c-block'})
                for article in articles: 
                    link = article.find('a', {'class': 'c-block__media-link'})
                    title = article.find('a', {'class': 'c-block__title-link'})
                    print(title.string.strip())
                    if title is None or title.string is None:
                        continue
                    article_url = OutsideArticleUrl(url="https://www.outsideonline.com" + link['href'], title=str(title.string.strip()) or '')
