BASE_URL="https://genius.com/articles?page="
N_ARTICLE_LINK_PAGES = 40
OUTPUT_FILE = 'article-lists/genius-article-urls.json'
WORKER_THREADS = 1
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                articles = soup.find_all('div', {'class': 'articles_list-item'})
                for article in articles: 
                    link = article.find('a', {'class': 'articles_list-item-link'})
                    title = article.find('div', {'class': 'articles_list-item-article_info-title'})
                    print(title.string.strip())
                    if title is None or title.string is None:
                        continue
                    article_url = GeniusArticleUrl(url="https://genius.com" + link['href'], title=str(title.string.strip()) or '')
