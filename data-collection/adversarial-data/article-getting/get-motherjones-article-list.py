BASE_URL="https://www.motherjones.com/topics/investigations/page/"
N_ARTICLE_LINK_PAGES = 11
OUTPUT_FILE = '../article-lists/motherjones-article-urls.json'
WORKER_THREADS = 10
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                articles = soup.find_all('li', {'class': 'article-item'})
                for article in articles: 
                    stuff = article.find('h3', {'class': 'hed'})
                    title = stuff.find('a')
                    print(title.string.strip())
                    if title is None or title.string is None:
                        continue
                    article_url = AeonArticleUrl(url=title['href'], title=str(title.string.strip()) or '')
