BASE_URL="https://www.rollingstone.com/culture/culture-features/page/"
N_ARTICLE_LINK_PAGES = 100
OUTPUT_FILE = 'article-lists/rollingstone-article-urls.json'
WORKER_THREADS = 16
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                articles = soup.find_all('li', {'class': 'l-river__item'})
                for article in articles: 
                    link = article.find('a', {'class': 'c-card__wrap'})
                    title = article.find('h3', {'class': 'c-card__heading t-bold'})
        
                    if title is None or title.string is None:
                        continue
                    article_url = RollingStoneArticleUrl(url=link['href'], title=str(title.string.strip()) or '')
