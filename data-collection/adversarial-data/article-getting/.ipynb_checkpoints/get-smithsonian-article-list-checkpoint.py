BASE_URL="https://www.smithsonianmag.com/category/history/?page="
N_ARTICLE_LINK_PAGES = 430
OUTPUT_FILE = '../article-lists/smithsonian-article-urls.json'
WORKER_THREADS = 32
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                articles = soup.find_all('section', {'class': 'article-teaser'})
                for article in articles: 
                    main = article.find('h3', {'class': 'headline'})
                    title = main.find('a')
                    if title is None or title.string is None:
                        continue
                    article_url = WiredArticleUrl(url="https://www.smithsonianmag.com"+ title['href'], title=str(title.string.strip()) or '')
