BASE_URL="https://www.theatlantic.com/latest/?page="
N_ARTICLE_LINK_PAGES = 100
OUTPUT_FILE = 'article-lists/theatlantic-article-urls.json'
WORKER_THREADS = 16
        ...
        for i in self.chunk:
            try:
                ...
                for article in articles: 
                    link = article.find('a')
                    title = article.find('h2', {'class': 'hed'})
        
                    if title is None or title.string is None:
                        continue
                    article_url = AtlanticArticleUrl(url="https://www.theatlantic.com" + link['href'], title=str(title.string.strip()) or '')
