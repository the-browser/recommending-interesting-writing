BASE_URL="https://www.vulture.com/blog/?start="
N_ARTICLE_LINK_PAGES = 75
OUTPUT_FILE = '../article-lists/vulture-article-urls.json'
WORKER_THREADS = 32
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                article_list_page = get(f"{BASE_URL}{i*50}")
                soup = BeautifulSoup(article_list_page.text, "html5lib")
                articles = soup.find_all('li', {'class': 'article'})
                for article in articles: 
                    link = article.find('a', {"class": "link-text"})
                    title = article['data-track-headline']
                    print(title.strip())
                    if title is None:
                        continue
                    article_url = AeonArticleUrl(url=link['href'], title=str(title.strip()) or '')
