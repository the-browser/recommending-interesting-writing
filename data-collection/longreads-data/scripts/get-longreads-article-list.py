BASE_URL="https://longreads.com/picks/page/"
N_ARTICLE_LINK_PAGES = 100
OUTPUT_FILE = '../article-lists/longreads-article-urls.json'
WORKER_THREADS = 16
        ...
        for i in self.chunk:
            try:
                ...
                for article in articles: 
                    stuff = article.find('h2', {'class': 'h-one article-title'})
                    info = stuff.find("a")
                    print(info.string.strip())
                    if info is None or info.string is None:
                        continue
                    article_url = LongReadsArticleUrl(url=info['href'], title=str(info.string.strip()) or '')
