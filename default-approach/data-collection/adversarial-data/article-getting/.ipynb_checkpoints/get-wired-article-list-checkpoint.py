BASE_URL="https://www.wired.com/most-recent/page/"
N_ARTICLE_LINK_PAGES = 100
OUTPUT_FILE = 'article-lists/wired-article-urls.json'
WORKER_THREADS = 16
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                articles = soup.find_all('li', {'class': 'archive-item-component'})
                for article in articles: 
                    link = article.find('a', {'class': 'archive-item-component__link'})
                    title = article.find('h2', {'class': 'archive-item-component__title'})
        
                    if title is None or title.string is None:
                        continue
                    article_url = WiredArticleUrl(url="https://www.wired.com"+link['href'], title=str(title.string.strip()) or '')
