BASE_URL="https://www.newyorker.com/latest/news/page/"
N_ARTICLE_LINK_PAGES = 100
OUTPUT_FILE = 'article-lists/newyorker-article-urls.json'
WORKER_THREADS = 16
        ...
        for i in self.chunk:
            try:
                ...
                for article in articles: 
                    link = article.find('a', {'class': 'Link__link___3dWao'})
                    title = article.find('h4', {'class': 'River__hed___re6RP'})
        
                    if title is None or title.string is None:
                        continue
                    article_url = NewYorkerArticleUrl(url="https://www.newyorker.com" + link['href'], title=str(title.string.strip()) or '')
