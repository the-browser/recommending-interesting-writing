BASE_URL="https://slate.com/culture/"
N_ARTICLE_LINK_PAGES = 200
OUTPUT_FILE = '../article-lists/slate-article-urls.json'
WORKER_THREADS = 32
    ...
    def run(self):
        for i in self.chunk:
            try:
                ...
                articles = soup.find_all('a', {'class': 'topic-story'})
                for article in articles: 
                    title = article.find('span', {'class': 'topic-story__hed'})
                    print(title.string.strip())
                    if title is None or title.string is None:
                        continue
                    article_url = AeonArticleUrl(url=article['href'], title=str(title.string.strip()) or '')
